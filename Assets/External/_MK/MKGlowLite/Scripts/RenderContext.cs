...
namespace MK.Glow
{
	...
	internal sealed class RenderContext : IDimension
	{
		...
		internal void UpdateRenderContext(CameraData cameraData, RenderTextureFormat format, int depthBufferBits, RenderDimension dimension)
        {
			#if UNITY_2018_3_OR_NEWER
			#if ENABLE_VR
			_descriptor.dimension = cameraData.stereoEnabled ? UnityEngine.XR.XRSettings.eyeTextureDesc.dimension : UnityEngine.Rendering.TextureDimension.Tex2D;
            _descriptor.vrUsage = cameraData.stereoEnabled ? UnityEngine.XR.XRSettings.eyeTextureDesc.vrUsage : VRTextureUsage.None;
            _descriptor.volumeDepth = cameraData.stereoEnabled ? UnityEngine.XR.XRSettings.eyeTextureDesc.volumeDepth : 1;
			#else
