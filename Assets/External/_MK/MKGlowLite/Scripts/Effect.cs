...
namespace MK.Glow
{
    ...
    internal sealed class Effect
    {
        ...
        {
            RenderDimension renderDimension = new RenderDimension((int)((float)_cameraData.width), (int)((float)_cameraData.height));
        ...
        internal void Build(RenderTarget source, RenderTarget destination, Settings settings, CommandBuffer cmd, CameraData cameraData, UnityEngine.Camera renderingCamera)
        {
            _commandBuffer = cmd;
            _settings = settings;
            ...
            if(_settings.workflow == Workflow.Selective)
            {
                ...
                _selectiveRenderTarget.renderTexture = RenderTexture.GetTemporary((int)((float)_cameraData.width) / 2, (int)((float)_cameraData.height) / 2, 16, _renderTextureFormat, RenderTextureReadWrite.Default, 1);//PipelineExtensions.GetTemporary(_selectiveRenderContext, _renderTextureFormat);
